...
func TestJanitor_EvictionImmunity(t *testing.T) {
	...
	// 3. Insert Test Data
	// Shard 1: Core (IMMUNE)
	v.SaveShard(storage.Shard{ID: "core-1", Category: "core", Content: "Ego"})
	t.Log("Injected Core Shard (Immune by Category)")

	// Shard 2: Memory (IMMUNE by Bond)
	v.SaveShard(storage.Shard{ID: "bonded-1", Category: "memory", Content: "Deep Thought"})
	...
	v.SaveShard(storage.Shard{ID: "memory-1", Category: "memory", Content: "Active Memory"})
	...
	v.SaveShard(storage.Shard{ID: "orphan-1", Category: "memory", Content: "Temp Data"})
	...
	results, _ := v.FindResonant(nil, 10) // nil vector just gets everything
